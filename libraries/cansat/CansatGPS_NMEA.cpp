...
Cansat_GPS_NMEA::Cansat_GPS_NMEA(GPS_State &_state) :
...
*/
bool Cansat_GPS_NMEA::decode(char c)
{
...
// internal utilities
//
int16_t Cansat_GPS_NMEA::_from_hex(char a)
{
    if (a >= 'A' && a <= 'F')
        return a - 'A' + 10;
    else if (a >= 'a' && a <= 'f')
...
int32_t Cansat_GPS_NMEA::_parse_decimal_100(const char *p)
{
    char *endptr = nullptr;
 ...
 */
uint32_t Cansat_GPS_NMEA::_parse_degrees()
{
    char *p, *q;
    uint8_t deg = 0, min = 0;
    float frac_min = 0;
 ...
 */
bool Cansat_GPS_NMEA::_have_new_message()
{
    if (_last_RMC_ms == 0 ||
        _last_GGA_ms == 0) {
        return false;
...
bool Cansat_GPS_NMEA::_term_complete()
{
